...
*/
	if (mvolts < Vb_MIN )
		return (0);
	else if (mvolts > Vb_MAX )
		bv = 254;
	else
	{
		//bv = (mvolts-VDD_MIN) / ((VDD_BAT-VDD_MIN)/255);
	    //float one_division =/255;
        // bv = (uint16_t)result;
	    //float result = (mvolts-VDD_MIN) / one_division;

	    bv =  ((mvolts-Vb_MIN) * 255) /  (Vb_MAX-Vb_MIN);
//	    bv = (bv * 100) / 254;
	}
    bv = (bv * 100) / 254;
	return (bv);
